...
public:
	...
	// Operators:
	GameEntry& operator[](size_t i) {return entries[i];};	// Access to game entries
	void print();
...
ostream& operator<<(ostream& out, const Scores& obj){
	out << "numEntries=" << obj.numEntries << ", maxEntries=" << obj.maxEntries << endl;
	for (int i=0; i<obj.numEntries; i++)
		out << i << ": name=" << obj.entries[i].getName().c_str() << ", score=" << obj.entries[i].getScore() << endl;
	out << endl;
	return out;
}
